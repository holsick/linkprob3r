#!/usr/bin/python3
# version 1.1
# Output file and output file format still needs to be fixed
# cewl wordlist feature still needs to be fixed/implemented
# authentication options still needs to be implemented
...
# detects if the link is a subdomain of the target (lots of lists)
...
def getFormDetails(form):
  global method
  global action # fixes UnboundLocalError (needs permanent fix)
  details = {}
  try:
...
# grab all linked JavaScript files
def getJSFiles(lst):
  jsFile = []
...
if __name__ == '__main__':
  ...
  # request 2: process the return value of getLinks to avoid making too many unneeded requests
